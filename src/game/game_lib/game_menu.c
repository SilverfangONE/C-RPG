...
static uint64_t hash_Node_mobj(const void *item, uint64_t seed0, uint64_t seed1);
...
static MenuObject* init_mobj() 
{
    ...
    mobj->items = hashmap_new(sizeof(struct MenuObject), 0, 0, 0, hash_Node_mobj, compare_Node_mobj, NULL, NULL);
    return mobj;
}
...
MenuObject* create_mobj(char* name, int mod) 
{
    ...
    switch(mod) {
        ...
        case 1:
            ...
            return mobj;
        default:
            log_warn("Mode %d is not supported for create_mobj(char* name, int mod)!", mod);
            return NULL;
    }
}
...
{
    if(hashmap_set(mobjP->items, &(struct Node_mobj){ .key=mobjC->sName, .value=mobjC}) != NULL) 
    {
...
{
    struct Node_mobj* node = hashmap_get(mobj->items, &(struct Node_mobj){ .key=mobj->sName } );
    return node->value;
}
...
void Build_mobj(MenuObject* mobj) 
{
    ...
    {
        const struct MenuObject* mobj_Child = ((struct Node_mobj*)item)->value;
        if(hasChildren_mobj(mobj_Child)) {
            Build_mobj(mobj_Child);
        }

        // Longest child name determines cell width
        mobj->vCellSize.x = maxValue(getSize_mobj(mobj_Child).x, mobj->vCellSize.x );
        mobj->vCellSize.y = maxValue(getSize_mobj(mobj_Child).y, mobj->vCellSize.y );
    }

    // Adjust size of object (in patches) if it were rendered as a panel 
    mobj->vSizeInPatches.x = mobj->vCellTable.x * mobj->vCellSize.x + (mobj->vCellSize.x - 1) * mobj->vCellPadding.x + 2;
    mobj->vSizeInPatches.y = mobj->vCellTable.y * mobj->vCellSize.y + (mobj->vCellSize.y - 1) * mobj->vCellPadding.y + 2;

    // Calculate how many rows this item has to hold
    int itemsSize = hashmap_count(mobj->items);
    mobj->nTotalRows = (itemsSize / mobj->vCellTable.x) + (((itemsSize % mobj->vCellTable.x) > 0) ? 1 : 0);
}
