 ...
 */
static int (*flookupBackgroundTiles_UI_RPGE)(enum MENU_TILE_TYPE) = &default_getIndex_Background_MENU_TILE_UI_RPGE;
...
{
    if (newflookupBackgroundTiles_UI_RPGE == NULL)
        return 1;
...
{
    flookupBackgroundTiles_UI_RPGE = &default_getIndex_Background_MENU_TILE_UI_RPGE;
}
...
int render_Background_UI_RPGE(SDL_Renderer *renderer, Background_UI_RPGE *background)
{
    ...
    for (int yTable = 0; yTable < ySize; yTable++)
    {
        ...
        {
            Vec2D cor = {.x = background->vCoordinates.x + xTable * background->asset->vPatchSize.x,
                         .y = background->vCoordinates.y + yTable * background->asset->vPatchSize.y};
            if (renderTile_Assetsheet_G_RPGE(
                    renderer, background->asset,
                    getCell_ARRAY2D_UTIL(background->m_indices, background->vTableSize, xTable, yTable), cor))
            {
                return 1;
            }
        }
...
int *constructBackgroundIndexArr_UI_RPGE(Vec2D vTableSize)
{
    ...
    setCell_ARRAY2D_UTIL(m_indices, vTableSize, vTableSize.x - 1, 0,
                         lookup_BackgroundTiles_UI_RPGE(MENU_UP_RIGTH_CORN));
    setCell_ARRAY2D_UTIL(m_indices, vTableSize, 0, vTableSize.y - 1,
                         lookup_BackgroundTiles_UI_RPGE(MENU_DOWN_LEFT_CORN));
    setCell_ARRAY2D_UTIL(m_indices, vTableSize, vTableSize.x - 1, vTableSize.y - 1,
                         lookup_BackgroundTiles_UI_RPGE(MENU_DOWN_RIGTH_CORN));
    // set boarders.
    // sides.
    for (int y = 1; y < vTableSize.y - 1; y++)
    {
        ...
        // rigth
        setCell_ARRAY2D_UTIL(m_indices, vTableSize, vTableSize.x - 1, y,
                             lookup_BackgroundTiles_UI_RPGE(MENU_EAST_BOARDER_MID));
    }
    // top and down
    for (int x = 1; x < vTableSize.x - 1; x++)
    {
        ...
        // down
        setCell_ARRAY2D_UTIL(m_indices, vTableSize, x, vTableSize.y - 1,
                             lookup_BackgroundTiles_UI_RPGE(MENU_SOUTH_BOARRDER_MID));
    }
    return m_indices;
}
