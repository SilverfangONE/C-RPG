...
renderGame (GameState *game)
{
  ...
  */
  ...
  if (!SDL_RenderTexture (game->renderer, game->display.texture, NULL,
                          &game->display.destRect))
    {
      log_error ("%s", SDL_GetError ());
    }
...
// ---- TILES & SPRITE ----
void
renderMatrixMap (GameState *game, Matrix *matrix, TextureAtlas *atlas)
{
      ...
      for (int col = 0; col < matrix->cols; col++)
        {
          ...
          renderTextureFromAtlas (
              game, atlas, getMatrixCell (matrix, col, row) - 1,
              col * atlas->tileSizeX, row * atlas->tileSizeY);
        }
    }
}

void
renderTextureFromAtlas (GameState *game, TextureAtlas *atlas, int tileIndex,
                        int x, int y)
{
...
void
renderPlayer (GameState *game, TextureAtlas *atlas)
{
  renderTextureFromAtlas (game, atlas, game->player->textureAtlasIndex,
                          game->player->x, game->player->y);
}

void
renderEnviromentStack (GameState *game, struct ENV_StackItem *item)
{
  if (item->next != NULL)
    {
      return renderEnviromentStack (game, item->next);
    }
  struct ENV *env = item->env;
