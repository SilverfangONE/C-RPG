...
main ()
{
  ...
  if (!SDL_CreateWindowAndRenderer ("Editor", EDITOR_WINDOW_WIDTH,
                                    EDITOR_WINDOW_HEIGHT, 0, &context->window,
                                    &context->renderer))
    {
      log_error ("%s", SDL_GetError ());
      terminate (context);
    }
  reset_Keymap (&context->keymap);
  context->display
      = create_Display (context->renderer, SNES_PIXEL_WIDTH, SNES_PIXEL_HEIGHT,
...
void
processEditorEventsSDL (EditorContext *context)
{
  reset_KeymapExceptPressed (&context->keymap);
  SDL_Event event;
...
void
update (EditorContext *context)
{
  // TODO.
}

void
render (EditorContext *context)
{
  // render grid;
  // render tilesheet;
  // render menu stack;
...
void
renderMenuStack (struct MenuLayerStackItem *item)
{
  if (item->next != NULL)
    {
...
void
renderMenuUI (struct MenuUI *ui)
{
}

/* Terminates Editor Context*/
void
terminate (EditorContext *context)
{
...
// &&&& Load and Destory &&&&
void
loadSettingsFromEnvJSON (EditorContext *context)
{
  return;
}
